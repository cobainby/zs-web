      ...
      <el-card class="box-card" shadow="never" v-loading="loading">
        ...
        <el-tabs v-model="activeName" @tab-click="handleClick" style="margin-top:-10px;">
          <el-tab-pane class="chartsPanel" label="成果数据" name="first-ta">
            ...
            <el-table :data="selectDatas" border :default-sort="{prop:'surveyTime', order: 'descending'}"  :row-style="tableRowStyle" :header-cell-style="tableHeaderStyle" style="width: 100%;" :height="tableHeight" @selection-change="handleSelectionChange">
              ...
              <el-table-column align="center" label="支撑轴力" :show-overflow-tooltip="true">
                <template slot-scope="scope">
          ...
          <el-tab-pane label="单次变化量" name="second-ta">
            <div :class="className" id="lfLineGap"></div>
          </el-tab-pane>
          <el-tab-pane label="累计变化量" name="third-ta">
            <div :class="className" id="lfLineAccum"></div>
          </el-tab-pane>
        </el-tabs>
      </el-card>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      allItems: new Array(), //所有lf的数据
      lfPoints: [], //所有lf点的集合
      lastLfDatas: [], //lf的最新一条数据
      selectDatas: [], //lf的勾选时间以后的表格数据，未限定时间时与lastLfDatas相等
      pagination: {
        total: 0,
        pageSize: 20,
  ...
  methods: {
    ...
    handleClick: function(tab, event) {
      $("#lfLineGap").width($(".chartsPanel").width());
      $("#lfLineGap").height($(window).height() - 180);
      $("#lfLineAccum").width($(".chartsPanel").width());
      $("#lfLineAccum").height($(window).height() - 180);
      this.initChart();
    },
    //图形展示
    initChart() {
      var myChartGap = echarts.init(
        document.getElementById("lfLineGap"),
        "macarons"
      );
      var myChartAccum = echarts.init(
        document.getElementById("lfLineAccum"),
        "macarons"
      );
      var option = {
        tooltip: {
    ...
    init(page, limit) {
      ...
      getHorizontal({
        ...
        this.lfPoints = lfPoints; //点的集合
        this.lastVarySeries = []; //单次变化量的曲线图数据源集合初始化一次
        this.accumVarySeries=[];//累计变化量的曲线图数据初始化一次
        this.lastLfDatas = []; //初始化一次
        for (var k = 0; k < this.lfPoints.length; k++) {
          var lfDatas = this.allItems[this.lfPoints[k]]; //每个点的数据
          var lastData = lfDatas[lfDatas.length - 1]; //每个点的最新数据
          if (lastData == undefined) {
            lastData = new Object();
          }
          lastData["pointCode"] = this.lfPoints[k];
          this.lastLfDatas.push(lastData); //最新一条数据的集合，用于表格
          this.selectDatas = this.lastLfDatas; //初始化为限定时间时，表格数据就是所有点的最新数据
          //折线图数据源的获取
          var lastVaryData = []; //单次变化量的data二维数组，存放时间和单次变化量
          var accumVaryData = []; //累计变化量的data二维数组，存放时间和累计变化量
          var singleTime = new Object(); //测量时间
          for (var j = 0; j < lfDatas.length; j++) {
            singleTime = this.changeTimeFormat(lfDatas[j].surveyTime); //格式化时间
            lastVaryData[j] = [singleTime, lfDatas[j].lastVary];
            accumVaryData[j] = [singleTime, lfDatas[j].accumVary];
          ...
          //构造echart的series格式
          lastVarySingle["name"] = this.lfPoints[k];
          ...
          accumVarySingle["name"] = this.lfPoints[k];
    ...
    handleRest() {
      $("#startTime").val("");
      $("#endTime").val("");
      this.selectDatas = this.lastLfDatas;
    },
    //根据时间查询最新数据
    handleSearch() {
      debugger;
      ...
      var newTime;
      for (var i = 0; i < this.lastLfDatas.length; i++) {
        newTime = new Date(this.lastLfDatas[i].surveyTime).getTime();
        if (newTime >= beforeTime && newTime <= afterTime) {
          timesData.push(this.lastLfDatas[i]);
        }
      }
