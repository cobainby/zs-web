      ...
      <el-card class="box-card" shadow="never" v-loading="loading">
        ...
        <el-tabs v-model="activeName" @tab-click="handleClick" style="margin-top:-10px;">
          ...
          <el-tab-pane label="单次变化量" name="second-ta">
            <div :class="className" id="wydLineGap"></div>
          </el-tab-pane>
          <el-tab-pane label="累计变化量" name="third-ta">
            <div :class="className" id="wydLineAccum"></div>
          </el-tab-pane>
        </el-tabs>
      </el-card>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      allItems: new Array(), //所有wyd的数据
      wydPoints: [], //所有wyd点的集合
      lastWydDatas: [], //wyd的最新一条数据
      selectDatas: [], //wyd的勾选时间以后的表格数据，未限定时间时与lastWydDatas相等
      pagination: {
        total: 0,
        pageSize: 20,
  ...
  methods: {
    ...
    initChart() {
      var myChartGap = echarts.init(
        document.getElementById("wydLineGap"),
        "macarons"
      );
      var myChartAccum = echarts.init(
        document.getElementById("wydLineAccum"),
        "macarons"
      );
      var option = {
        tooltip: {
    ...
    init(page, limit) {
      ...
      getVertical({
        ...
        this.wydPoints = wydPoints; //点的集合
        this.lastVarySeries = []; //单次变化量的曲线图数据源集合初始化一次
        this.accumVarySeries=[];//累计变化量的曲线图数据初始化一次
        this.lastWydDatas = []; //初始化一次
        for (var k = 0; k < this.wydPoints.length; k++) {
          var wydDatas = this.allItems[this.wydPoints[k]]; //每个点的数据
          var lastData = wydDatas[wydDatas.length - 1]; //每个点的最新数据
          if (lastData == undefined) {
            lastData = new Object();
          }
          lastData["pointCode"] = this.wydPoints[k];
          ...
          //构造echart的series格式
          lastVarySingle["name"] = this.wydPoints[k];
          ...
          accumVarySingle["name"] = this.wydPoints[k];
    ...
    handleRest() {
      $("#startTime").val("");
      $("#endTime").val("");
      this.selectDatas = this.lastWydDatas;
    },
    //根据时间查询最新数据
    handleSearch() {
      debugger;
      ...
      var newTime;
      for (var i = 0; i < this.lastWydDatas.length; i++) {
        newTime = new Date(this.lastWydDatas[i].surveyTime).getTime();
        if (newTime >= beforeTime && newTime <= afterTime) {
          timesData.push(this.lastWydDatas[i]);
        }
      }
