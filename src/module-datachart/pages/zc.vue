      ...
      <el-card class="box-card" shadow="never" v-loading="loading">
        ...
        <el-tabs v-model="activeName" @tab-click="handleClick" style="margin-top:-10px;">
          ...
          <el-tab-pane label="单次变化量" name="second-ta">
            <div :class="className" id="zcLineGap"></div>
          </el-tab-pane>
          <el-tab-pane label="累计变化量" name="third-ta">
            <div :class="className" id="zcLineAccum"></div>
          </el-tab-pane>
        </el-tabs>
      </el-card>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      zcPoints: [], //所有zc点的集合
      lastZcDatas: [], //zc的最新一条数据
      selectDatas: [], //zc的勾选时间以后的表格数据，未限定时间时与lastZcDatas相等
      pagination: {
        total: 0,
        pageSize: 20,
  ...
  methods: {
    ...
    handleClick: function(tab, event) {
      $("#zcLineGap").width($(".chartsPanel").width());
      $("#zcLineGap").height($(window).height() - 180);
      $("#zcLineAccum").width($(".chartsPanel").width());
      $("#zcLineAccum").height($(window).height() - 180);
      this.initChart();
    },
    //图形展示
    initChart() {
      var myChartGap = echarts.init(
        document.getElementById("zcLineGap"),
        "macarons"
      );
      var myChartAccum = echarts.init(
        document.getElementById("zcLineAccum"),
        "macarons"
      );
      var option = {
        tooltip: {
    ...
    init(page, limit) {
      ...
      getForce({
        ...
        for (var k = 0; k < this.zcPoints.length; k++) {
          ...
          this.lastZcDatas.push(lastData); //最新一条数据的集合，用于表格
          this.selectDatas = this.lastZcDatas; //初始化为限定时间时，表格数据就是所有点的最新数据
          //折线图数据源的获取
          ...
          //构造echart的series格式
          lastVarySingle["name"] = this.zcPoints[k];
          ...
          accumVarySingle["name"] = this.zcPoints[k];
    ...
    handleRest() {
      $("#startTime").val("");
      $("#endTime").val("");
      this.selectDatas = this.lastZcDatas;
    },
    //根据时间查询最新数据
    handleSearch() {
      debugger;
      ...
      var newTime;
      for (var i = 0; i < this.lastZcDatas.length; i++) {
        newTime = new Date(this.lastZcDatas[i].surveyTime).getTime();
        if (newTime >= beforeTime && newTime <= afterTime) {
          timesData.push(this.lastZcDatas[i]);
        }
      }
