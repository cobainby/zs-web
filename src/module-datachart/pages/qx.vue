      ...
      <el-card class="box-card" shadow="never" v-loading="loading">
        ...
        <el-tabs v-model="activeName" @tab-click="handleClick" style="margin-top:-10px;">
          ...
          <el-tab-pane label="单次变化量" name="second-ta">
            <div :class="className" id="qxLineGap"></div>
          </el-tab-pane>
          <el-tab-pane label="累计变化量" name="third-ta">
            <div :class="className" id="qxLineAccum"></div>
          </el-tab-pane>
        </el-tabs>
      </el-card>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      allItems: new Array(), //所有qx的数据
      qxPoints: [], //所有qx点的集合
      lastQxDatas: [], //qx的最新一条数据
      selectDatas: [], //qx的勾选时间以后的表格数据，未限定时间时与lastQxDatas相等
      pagination: {
        total: 0,
        pageSize: 20,
  ...
  methods: {
    ...
    handleClick: function(tab, event) {
      $("#qxLineGap").width($(".chartsPanel").width());
      $("#qxLineGap").height($(window).height() - 180);
      $("#qxLineAccum").width($(".chartsPanel").width());
      $("#qxLineAccum").height($(window).height() - 180);
      this.initChart();
    },
    //图形展示
    initChart() {
      var myChartGap = echarts.init(
        document.getElementById("qxLineGap"),
        "macarons"
      );
      var myChartAccum = echarts.init(
        document.getElementById("qxLineAccum"),
        "macarons"
      );
      var option = {
        tooltip: {
    ...
    init(page, limit) {
      ...
      getHorizontal({
        ...
        this.qxPoints = qxPoints; //点的集合
        this.lastVarySeries = []; //单次变化量的曲线图数据源集合初始化一次
        this.accumVarySeries=[];//累计变化量的曲线图数据初始化一次
        this.lastQxDatas = []; //初始化一次
        for (var k = 0; k < this.qxPoints.length; k++) {
          var qxDatas = this.allItems[this.qxPoints[k]]; //每个点的数据
          var lastData = qxDatas[qxDatas.length - 1]; //每个点的最新数据
          if (lastData == undefined) {
            lastData = new Object();
          }
          lastData["pointCode"] = this.qxPoints[k];
          this.lastQxDatas.push(lastData); //最新一条数据的集合，用于表格
          this.selectDatas = this.lastQxDatas; //初始化为限定时间时，表格数据就是所有点的最新数据
          //折线图数据源的获取
          var lastVaryData = []; //单次变化量的data二维数组，存放时间和单次变化量
          var accumVaryData = []; //累计变化量的data二维数组，存放时间和累计变化量
          var singleTime = new Object(); //测量时间
          for (var j = 0; j < qxDatas.length; j++) {
            singleTime = this.changeTimeFormat(qxDatas[j].surveyTime); //格式化时间
            lastVaryData[j] = [singleTime, qxDatas[j].rateVary];
            accumVaryData[j] = [singleTime, qxDatas[j].accumVary];
          ...
          //构造echart的series格式
          lastVarySingle["name"] = this.qxPoints[k];
          ...
          accumVarySingle["name"] = this.qxPoints[k];
    ...
    handleRest() {
      $("#startTime").val("");
      $("#endTime").val("");
      this.selectDatas = this.lastQxDatas;
    },
    //根据时间查询最新数据
    handleSearch() {
      debugger;
      ...
      var newTime;
      for (var i = 0; i < this.lastQxDatas.length; i++) {
        newTime = new Date(this.lastQxDatas[i].surveyTime).getTime();
        if (newTime >= beforeTime && newTime <= afterTime) {
          timesData.push(this.lastQxDatas[i]);
        }
      }
