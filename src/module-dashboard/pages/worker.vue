      ...
      <el-card shadow="never">
        ...
        <el-table :key='tableKey' :data="dataList" fit highlight-current-row style="width: 100%" border>
          ...
          <el-table-column align="center" label="专业">
            <template slot-scope="scope">
              <span>{{scope.row.titleMajor}}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column align="center" label="证件编号">
            <template slot-scope="scope">
          ...
          <el-table-column align="center" label="权限等级">
            <template slot-scope="scope">
              <span v-if="scope.row.account!==null">{{scope.row.account.role.roleName}}</span>
              <span v-if="scope.row.account==null">无账户</span>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getList(page = 1, limit = 20) {
      ...
      // list({page, limit})
      axios
        .get("http://192.168.1.13:8181/static/getWorkers.json")
        .then(res => {
