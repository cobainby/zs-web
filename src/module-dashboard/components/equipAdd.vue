<template>
  <div class="add-form">
    <el-dialog width="40%" :title="text+pageTitle" :visible.sync="dialogFormVisible">
      <el-form :rules="ruleInline2" ref="formBase2" :model="formBase2" label-position="right" label-width="90px" style='margin:0 40px;'>
        <el-row>
          <el-col :span="12">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button icon="el-icon-success" size="mini" type="primary" @click="createData('formBase2')">{{$t('table.confirm')}}</el-button>
        <el-button icon="el-icon-edit-outline" size="mini" type="danger" @click="resetForm('formBase2')">重置</el-button>
        <el-button icon="el-icon-error" size="mini" @click="handleClose('formBase2')">{{$t('table.cancel')}}</el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    // 表单提交
    createData(formBase2) {
      this.$refs[formBase2].validate(valid => {
        if (valid) {
          debugger;
          let data = {
            ...this.formBase2
          ...
          if(data.equipStatus=="停用"){
            data.equipStatus=0;
          }else if(data.equipStatus=="启用"){
            data.equipStatus=1;
          }
          var equipData = new Object();
          equipData.token = getToken();
          equipData.data = data;
          //录入人的Uuid
          equipData.data.entryAccUuid=this.userUuid;
          //注意：这里重新赋值，用了el-date-picker将json时间格式化了
          equipData.data.verifyDate =
            $("#verifyDate").val() != "" ? $("#verifyDate").val() : null;
          equipData.data.verifyValidDate =
            $("#verifyValidDate").val() != ""? $("#verifyValidDate").val(): null;
          equipData.data.entryDate =
            $("#entryDate").val() != "" ? $("#entryDate").val() : null;
          equipData.data.confirmDate =
            $("#confirmDate").val() != "" ? $("#confirmDate").val() : null;
          if (this.text == "创建") {
            addEquip(equipData)
              .then(response => {
