<template>
  <div class="add-form">
    <el-dialog width="50%" title="监测点参数设置" :visible.sync="dialogFormVisible">
      <el-dialog width="40%" title="监测点参数" :visible.sync="innerVisible" append-to-body>
        <el-form ref="formData" :model="formData" label-position="right" label-width="100px" style='margin:0 20px;'>
          <el-row>
            <el-col :span="12">
              <el-form-item label="测点编号" prop="surveypointNumber">
                <el-input v-model="formData.surveypointNumber" placeholder="Please input"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="初始累计值" prop="accumInitial">
                <el-input v-model="formData.accumInitial" placeholder="Please input"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="12">
              <el-form-item label="测点状态" prop="surveypointCondition">
                <el-select v-model="formData.surveypointCondition" placeholder="please input">
                  <el-option id="surveypointCondition" v-for="item in option" :key="item.value" :label="item.label" :value="item.value" :disabled="item.disabled">
                  </el-option>
                </el-select>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="报警设置" prop="warningUuid">
                <el-select v-model="formData.warningUuid" placeholder="please input">
                  <el-option id="warningUuid" v-for="item in warningSelect" :key="item.warningUuid" :label="item.warningName" :value="item.warningUuid" :disabled="item.disabled">
                  </el-option>
                </el-select>
              </el-form-item>
            </el-col>
          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="排序号" prop="surveypointOrder">
                <el-input v-model="formData.surveypointOrder" placeholder="Please input"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="设置时间" prop="dateSet">
                <el-date-picker id="dateSet" v-model="formData.dateSet" type="datetime" placeholder="选择日期时间" default-time="12:00:00">
                </el-date-picker>
              </el-form-item>
            </el-col>
          </el-row>
      ...
      <el-button type="primary" size="small" icon="el-icon-circle-plus" style="margin-bottom:10px;" @click="addSuveyPoint">新增</el-button>
      <el-table :key='tableKey' :height="tableHeight" :data="surveypointSetData" highlight-current-row border>
        <el-table-column align="center" width="80px" label="测点编号">
        ...
        <el-table-column align="center" width="70px" label="排序号">
          <template slot-scope="scope">
        ...
        <el-table-column align="center" width="200px" label="操作" fixed="right">
          <template slot-scope="scope">
            <el-button :disabled="scope.row.is_deleted===1" type="primary" size="mini" icon="el-icon-edit" @click="updateSuveyPoint(scope.row)">修改</el-button>
            <el-button :disabled="scope.row.is_deleted===1" size="mini" type="danger" icon="el-icon-delete" @click="surveypointRemove(scope.row.surveypointUuid)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <div slot="footer" class="dialog-footer">
  ...
  methods: {
    ...
    dialogFormV(monitorItemUuid) {
      this.monitorItemUuid = monitorItemUuid;
       getWarningSet({monitorItemUuid:this.monitorItemUuid,token:this.token}).then(res=>{
         debugger
    ...
    updateSuveyPoint(params) {
      ...
      this.formData.surveypointCondition = params.surveypointCondition==0?"未启用":"启用";
